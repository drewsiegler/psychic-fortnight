...
async function saveToStorage(tasks, collapsedIds) {
  if (!window.storage) return;
  try {
    await window.storage.set(STORAGE_KEY, JSON.stringify({
      tasks: serializeTasks(tasks), collapsed: Array.from(collapsedIds),
...
async function clearStorage() {
  if (!window.storage) return;
  try { await window.storage.delete(STORAGE_KEY); } catch (e) { console.error(e); }
}

// ── Drag helpers ──
function getTaskBlock(tasks, index) {
  // Returns array of indices: the task + all its children
...
export default function GanttPlanner() {
  ...
  const [dropVisIdx, setDropVisIdx] = useState(null); // visible index to drop BEFORE
  ...
  // Auto-save
  useEffect(() => {
    if (isInitialLoad.current || loadState === "loading") return;
    if (saveTimer.current) clearTimeout(saveTimer.current);
    setSaveStatus("saving");
    saveTimer.current = setTimeout(async () => {
      await saveToStorage(tasks, collapsed);
      setSaveStatus("saved");
      setTimeout(() => setSaveStatus(""), 2000);
    }, 600);
    return () => { if (saveTimer.current) clearTimeout(saveTimer.current); };
  }, [tasks, collapsed, loadState]);
  ...
  const handleDragOver = useCallback((e, visIdx) => {
    ...
    // Determine if we should drop above or below this row
    const rect = e.currentTarget.getBoundingClientRect();
    const midY = rect.top + rect.height / 2;
    const dropBefore = e.clientY < midY ? visIdx : visIdx + 1;
    setDropVisIdx(dropBefore);
  }, []);
  ...
  const handleDrop = useCallback((e) => {
    ...
    setTasks(prev => {
      ...
      // Get the block of tasks to move (task + children)
      const block = getTaskBlock(prev, dragIdx);
      const blockIds = new Set(block.map(i => prev[i].id));

      // Compute the real target index in the full array
      // dropVisIdx is the position in visibleIndices
      let targetRealIdx;
      if (dropVisIdx >= visibleIndices.length) {
        targetRealIdx = prev.length; // drop at end
      } else {
        targetRealIdx = visibleIndices[dropVisIdx];
      }

      // Don't drop onto self
      if (blockIds.has(prev[targetRealIdx]?.id)) { return prev; }

      // Extract the block items
      ...
      for (const bi of block) {
        if (bi < targetRealIdx) adjustedTarget--;
      }
  ...
  // Determine which tasks are being dragged (for visual dimming)
  ...
  if (loadState === "loading") {
    return (
      <div style={{
        ...
        background: "var(--bg-primary, #fafafa)", color: "var(--text-secondary, #64748b)",
      }}>
        <div style={{
        ...
        <div style={{ fontSize: 14 }}>Loading your plan...</div>
  ...
  return (
    <div style={{
      ...
      height: "100vh", display: "flex", flexDirection: "column",
      background: "var(--bg-primary, #fafafa)", color: "var(--text-primary, #1a1a2e)",
    }}>
      {/* Header */}
      <div style={{
        padding: "14px 24px", borderBottom: "1px solid var(--border, #e2e2e8)",
        display: "flex", alignItems: "center", gap: 12,
        background: "var(--bg-surface, #fff)", flexShrink: 0,
      }}>
        <div style={{
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          {hasAnyParents && (<>
            <button onClick={() => {
              ...
              border: "1px solid var(--border, #d0d0d8)", borderRadius: 5,
              background: "transparent", cursor: "pointer",
              color: "var(--text-secondary, #64748b)", fontFamily: "inherit",
            }}>⊟ Collapse</button>
          ...
          {!showClearConfirm ? (
            <button onClick={() => setShowClearConfirm(true)} style={{
              ...
              border: "1px solid var(--border, #d0d0d8)", borderRadius: 5,
              background: "transparent", cursor: "pointer",
              color: "var(--text-tertiary, #94a3b8)", fontFamily: "inherit", marginLeft: 4,
            }}>Clear All</button>
          ) : (
            <div style={{
              ...
              <button onClick={() => setShowClearConfirm(false)} style={{
                ...
                color: "var(--text-secondary, #64748b)", cursor: "pointer", fontFamily: "inherit",
              }}>No</button>
            </div>
          )}
        </div>
      ...
      {/* Help bar */}
      <div style={{
        padding: "7px 24px", background: "var(--bg-subtle, #f1f0fb)",
        borderBottom: "1px solid var(--border, #e2e2e8)",
        ...
        <span style={{ borderLeft: "1px solid var(--border, #d0d0d8)", paddingLeft: 14 }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          borderRight: hasDates ? "1px solid var(--border, #e2e2e8)" : "none",
          overflow: "auto", transition: "width 0.3s ease",
          ...
          <div style={{ padding: "4px 0" }}>
            <div style={{
              ...
              borderBottom: "1px solid var(--border, #e2e2e8)",
              position: "sticky", top: 0, background: "var(--bg-surface, #fff)", zIndex: 2,
            }}>
                ...
                <div key={task.id} style={{ position: "relative" }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      minHeight: ROW_H,
                      borderBottom: "1px solid var(--border-light, #f0f0f5)",
                  ...
                  >
                    ...
                    <div
                      ...
                      onMouseLeave={e => { e.currentTarget.style.color = "var(--text-tertiary, #bbb)"; e.currentTarget.style.background = "transparent"; }}
                    ...
                    <div style={{ width: 110, position: "relative" }}>
                      <input
                        ...
                        style={{
                          ...
                          color: task.startDate ? "var(--text-primary, #1a1a2e)" : "var(--text-tertiary, #94a3b8)",
                    ...
                    <div style={{ width: 110, position: "relative" }}>
                      <input
                        ...
                        style={{
                          ...
                          outline: "none", padding: "5px 4px", fontSize: 12,
                          background: "transparent",
                          color: task.dueDate ? "var(--text-primary, #1a1a2e)" : "var(--text-tertiary, #94a3b8)",
                          fontFamily: "inherit", textAlign: "center", transition: "border-color 0.15s",
                        }}
                    ...
                    <div style={{
                      ...
                      {isParent && !isCollapsed ? (
                        <div style={{
                          ...
                          background: "var(--border-light, #e8e8ee)", overflow: "hidden",
                        }}>
                          <div style={{
                    ...
                    <button onClick={() => deleteTask(task.id)} style={{
                      ...
                      color: "var(--text-tertiary, #94a3b8)", cursor: "pointer", borderRadius: 4,
                      display: "flex", alignItems: "center", justifyContent: "center",
            ...
            >
              <button
                ...
                style={{
                  ...
                  border: "1px dashed var(--border, #d0d0d8)", borderRadius: 6,
                  background: "transparent", color: "var(--text-secondary, #64748b)",
                  cursor: "pointer", fontSize: 13, fontFamily: "inherit",
                }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = "#6366f1"; e.currentTarget.style.color = "#6366f1"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--border, #d0d0d8)"; e.currentTarget.style.color = "var(--text-secondary, #64748b)"; }}
              >+ Add task</button>
            </div>
          </div>
        </div>
        ...
        {/* Gantt chart */}
        {hasDates && (
          <div style={{ flex: 1, overflow: "auto", background: "var(--bg-primary, #fafafa)" }}>
            <div style={{
              ...
              {dateLabels.map((d, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    borderRight: "1px solid var(--border-light, #f0f0f5)",
                    background: isToday ? "rgba(99,102,241,0.08)" : isWeekend ? "rgba(0,0,0,0.02)" : "transparent",
                    color: isToday ? "#6366f1" : isWeekend ? "var(--text-tertiary, #aaa)" : "var(--text-secondary, #64748b)",
